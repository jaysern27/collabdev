...
class RankingReportRepository {
  ...
  Future<String> _getAttractionName(String? attractionId) async {
    ...
    try {
      ...
      if (doc.exists && doc.data() != null) {
        return (doc.data()?['name'] ?? attractionId).toString();
      }
    } catch (_) {}
  ...
  List<Map<String, dynamic>> _calculateRankings(
    ...
    rows.sort(
          (a, b) {
        // Sort by frequency first to make ranking more meaningful.
        final frequencyCompare =
        _positiveInt(
          b['frequency'],
          fallback: 0,
          allowZero: true,
        ...
        // If frequency is equal, compare priority score.
        final scoreCompare =
        _score(b).compareTo(
          _score(a),
        );
        ...
        // If still equal, compare severity.
        return _toDouble(
          b['severity'],
        ).compareTo(
          _toDouble(
